...
- Added support for rewriting paths to profile data files relative to
  `base_dir`, improving cache sharing between different directory trees. +
...
- Fixed path case handling on Windows, including Unicode-aware path comparison
  and matching of `ignore_headers_in_manifest` on case-insensitive file
  systems. +
  [small]#_[contributed by GitHub user trustytrojan and Joel Rosdahl]_#

- Fixed `base_dir` handling for include paths in preprocessor mode. +
  [small]#_[contributed by Joel Rosdahl]_#
